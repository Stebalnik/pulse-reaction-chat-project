...
The browser client now includes a public no-login MVP shell and an internal admin/debug surface. The backend now covers anonymous identity, profile save/load, adults-only room consent events, roulette matching, WebRTC signaling, active-match chat, moderation records, retention controls, privacy-safe analytics, and scoped admin tokens. Account-backed identity and richer production operations remain later milestones.
