...
The public room is adults-only and requires one combined acknowledgement before matching begins: age gate, safety terms, camera/microphone access intent, and consent to on-device physiological reaction-pattern analysis. The adults-only acknowledgement is posted to `POST /api/consent-events` when the backend is available, and the room reasserts the accepted policy when joining a server-backed session after a prior offline entry.

Physiological analysis is part of the required room-entry consent because the roulette product has no useful chat mode without reaction-pattern processing. After consent, the browser starts camera and microphone capture automatically, samples local face/skin ROI, computes baseline-relative neutral trend states on device, and uploads cleaned `reaction_outputs` with model/method version, confidence, quality score, reason codes, and region-agreement category. Users can pause call media immediately. The public room still does not share precise BPM with the peer by default.
...
The public room renders the call as one bounded primary video with a picture-in-picture secondary video instead of stretching video panes to fill large desktop/tablet screens. Users can swap peer/self placement with one click and switch between front and rear local cameras when a device exposes both. Front-camera self preview is mirrored locally; peer video is not mirrored. The local preview uses the same face ROI used by the pulse sampler to avoid showing the face larger than roughly 15% of the video frame when a valid ROI is available.
